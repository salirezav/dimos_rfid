...
for f in demo_blueprint.py go2_blueprints.py go2_agentic_blueprints.py msgs.py rfid_module.py _backend.py; do
    cp "${SCRIPT_DIR}/${f}" "${RFID_DIR}/${f}"
done

# Blueprint stubs in dimos/ must import from dimos.hardware.sensors.rfid, not dimos_rfid.
for f in demo_blueprint.py go2_blueprints.py go2_agentic_blueprints.py rfid_module.py; do
    sed -i \
